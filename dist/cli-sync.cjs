...
function loadConfig(configPath) {
  ...
  const fileMappings = raw.file_mappings.map((item, i) => validateFileMapping(item, i));
...
function validateFileMapping(raw, index) {
  if (!isRecord(raw)) {
...
function validateInternalConfig(raw) {
  if (!isRecord(raw)) {
...
function mapSourceToTarget(config, sourcePath) {
  for (const mapping of config.file_mappings) {
    if (mapping.source !== void 0 && mapping.target !== void 0) {
  ...
  for (const mapping of config.file_mappings) {
    if (mapping.source_dir !== void 0 && mapping.target_dir !== void 0) {
...
function filterFile(content, config) {
  let doc;
  try {
    doc = load(content);
  } catch {
  ...
  if (filtered === void 0 || filtered === null) {
    return null;
  }
  return dump(filtered, yamlDumpConfig);
}
function filterFiles(files, reachableFiles, config) {
...
function hasInternalMarker(node, config) {
  return isRecord(node) && node[config.internal_marker] === true;
}
function filterNode(node, config) {
  if (Array.isArray(node)) {
...
function stripFields(node, fields) {
  if (fields.length === 0) {
    return node;
  }
  if (Array.isArray(node)) {
...
function cleanDanglingRefs(node, excludedFiles, currentDir) {
  if (Array.isArray(node)) {
    ...
    for (const item of node) {
      ...
      if (itemResult.node !== void 0) {
        result2.push(itemResult.node);
      }
    }
    return {
      node: result2.length > 0 ? result2 : void 0,
      changed: result2.length !== node.length || changed2
    };
  }
  if (!isRecord(node)) {
  ...
  if (typeof node["$ref"] === "string") {
    ...
    if (!ref.startsWith("#")) {
      ...
      if (excludedFiles.has(resolved)) {
        ...
        return { node: void 0, changed: true };
      }
    }
  }
  const result = {};
...
// src/sync.ts
async function syncMultiFile(config, sourceRoot) {
  ...
  for (const filePath of reachableFiles) {
    const absPath = path6.resolve(sourceRoot, filePath);
    sourceFiles.set(filePath, fs5.readFileSync(absPath, "utf-8"));
  }
...
function syncBundled(config, sourceRoot) {
  const absPath = path6.resolve(sourceRoot, config.entrypoint);
...
function writeFiles(files, targetRoot) {
  for (const [targetPath, content] of files) {
    const absPath = path7.resolve(targetRoot, targetPath);
    const dir = path7.dirname(absPath);
    fs6.mkdirSync(dir, { recursive: true });
    fs6.writeFileSync(absPath, content, "utf-8");
  }
}
function deleteFiles(filePaths, targetRoot) {
  for (const filePath of filePaths) {
    const absPath = path7.resolve(targetRoot, filePath);
  ...
  for (const dir of sortedDirs) {
    const absDir = path7.resolve(targetRoot, dir);
    if (fs6.existsSync(absDir)) {
      const entries = fs6.readdirSync(absDir);
      if (entries.length === 0) {
        fs6.rmdirSync(absDir);
      }
    }
  }
}
